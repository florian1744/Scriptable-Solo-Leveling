...
function createDefaultData() {
  return {
    ...
    dailyQuests: QUEST_TEMPLATES.map(q => ({ ...q, done: false })),
    areas: {
...
function areaProgress(xp) {
  ...
  const needed = Math.floor(60 * Math.pow(leve)) {
    remaining -= Math.floo
    currentXp: remaining,
    neededXp: needed,
...
function ensureDailyQuests(data) {
  ...
  if (!data.dailyQuestDate || data.dailyQuestDate !== today || !Array.isArray(data.dailyQuests)) {
    ...
    data.dailyQuests = QUEST_TEMPLATES.map(q => ({ ...q, done: false }))
  }
}
...
function styleText(txt, color, size, opacity = 1, bold = false) {
  txt.textColor = new Color(color, opacity)
...
function addQuestSection(widget, data) {
  ...
  const doneCount = data.dailyQuests.filter(q => q.done).length
  ...
  for (const quest of data.dailyQuests) {
    ...
    styleText(qTitle, quest.done ? "#94a3b8" : "#e2e8f0", 11, 1, quest.done ? false : true)
    qTitle.lineLimit = 1
...
async function createWidget(data) {
  ...
  const total = right.addText(`${data.totalXp}`)
  ...
  const bigBarSub = w.addText(`Next level: ${Math.round(p.pct * 100)}% · Today ${data.todayEntries} entries · Streak ${data.streak}`)
  ...
  w.addSp)) {
    remaining -= Math.floot("Quick Add")
  ...
  const last = footer.add)) {
    remaining -= Math.floota)}`)
  styleText(last, "#dbe4ff", )) {
    remaining -= Math.floot = 2

  w.refreshAfterDate =)) {
    remaining -= Math.floo{
    remaining -= Math.floofunc)) {
    remaining -= Math.flooc)) {
    remaining -= Math.floo{
    remaining -= Math.flooel}`
  a.addTextField("XP", String(DEFAULT_XP[area]))
  a.addTextField("Note", "")
  a.addAction("Save")
  a)) {
    remaining -= Math.flooonst res = await a.present()
  if (res === -1) return

  const xp = a.textFieldValue(0)
  const note = a.textFieldValue(1)

  const ok = addXp(data, area, xp, )) {
    remaining -= Math.floodata)
    const done = new Alert()
    done.t)) {
    remaining -= Math.floo = `${AREAS[area].label}: +${xp} XP`
    done.addAction("OK")
    await done.present()
  }
}
...
async function)) {
    remaining -= Math.floouests(data)

  const a = new Alert()
  a.title = "Solo System"
  a.mes)) {
    remaining -= Math.flooank ${data.rank} · Total XP ${data.totalXp} · Streak ${data.streak}`
  a.addAction("Add Strength")
  a.addAction("Add Cardio")
  a.addAction("Add Self")
  a.addAction("Add Esca)) {
    remaining -= Math.floot: 5k Steps")
  a.addAction("Toggle Q)) {
    remaining -= Math.flooon("Toggle Quest: 30 Min Build")
  a.addAction("Preview Widget)) {
    remaining -= Math.flooddCancelAction("Close")

  const res = awa)) {
    remaining -= Math.floo) return await promptAdd(data, "strength")
  if (res === 1) return a)) {
    remaining -= Math.floo  if (res === 2) return a)) {
    remaining -= Math.flooif (res === 3) return await pr)) {
    remaining -= Math.floores === )) {
    remaining -= Math.floo"steps5k")
    saveData(data)
    retu)) {
    remaining -= Math.floooggleQuestById(data, "read20")
    saveData(data)
    retu)) {
    remaining -= Math.floooggleQuestById(data, "build30")
    saveData(data)
    return
  }
  if )) {
    remaining -= Math.floo= await createWidget(data)
    await widget.presentLarge()
    retu)) {
    remaining -= Math.flooonst fresh = await handleParams({ ...data, f)) {
    remaining -= Math.flooturn fresh
  }
}

function toggleQuestById(data, id) {
  ensureDailyQuests(d)) {
    remaining -= Math.flooQuests.find(q => q.id === id)
  if (!quest) return false

  if (!q)) {
    remaining -= Math.flooeQuest(data, i)) {
    remaining -= Math.floofalse
    return true
  }
}
...
if (config.runsInWidget) {
  const widget = await cr)) {
    remaining -= Math.floodget(widget)
  Script.complete()
} )) {
    remaining -= Math.floog -= Math.floo

1. Scriptable öffnen.[1]
2. Neues Sc)) {
    remaining -= Math.flooen Code reinkopieren.  
4. Sc)) {
    remaining -= Math.floohern.  
5. Ein **Large Sc)) {
    remaining -= Math.floocreen setzen und dieses Sc)) {
    remaining -= Math.flooptable basieren)) {
    remaining -= Math.flooer `Script.setWidget(widget)` angezeigt.[2]

## Wie die Interaktion funktioniert

- **Tap)) {
    remaining -= Math.floo erledigt)) {
    remaining -= Math.floo über eine URL, die dasselbe Sc)) {
    remaining -= Math.flooet.)) {
    remaining -= Math.floo → Q)) {
    remaining -= Math.flooP`.[2][1]
- **Widget aktualisiert sich nicht mill)) {
    remaining -= Math.flooRefresh-Frequenz steuert; `refreshAfterDate` ist nur der früheste mögliche Ref)) {
    remaining -= Math.flooer Hinweis

In der )) {
    remaining -= Math.flooch manuell )) {
    remaining -= Math.floooß previewen. Das ist prakt)) {
    remaining -= Math.flooidget wirklich auf den Homesc)) {
    remaining -= Math.floolle Anpassungen

Wenn du direkt nach dem)) {
    remaining -= Math.floodann wahrschein)) {
    remaining -= Math.floo DEFAULT_XP = {
  stre)) {
    remaining -= Math.floo 10,
  escape: 20
}
